...
function Cams() {
  ...
  const [selectedDeviceID, setSeletedDeviceID] = useState<string | undefined>(undefined)
  ...
  const getQrayStream: (selectedDeviceID: string | undefined) => void = async (
    ...
    try {
      await navigator.mediaDevices.enumerateDevices().then(devices => {
        ...
        devices.forEach(deviceInfo => {
          if (deviceInfo.kind === "videoinput" && deviceInfo.deviceId === selectedDeviceID) {
            const newElement = { deviceInfo, checked: true }
            newDeviceList.push(newElement)
          }
          if (deviceInfo.kind === "videoinput" && deviceInfo.deviceId != selectedDeviceID) {
            const newElement = { deviceInfo, checked: false }
            newDeviceList.push(newElement)
          }
        })
  ...
  const handleCheckboxChange = (deviceId: string) => {
    ...
    if (deviceId === selectedDeviceID) {
      ...
      deviceList.forEach(device => {
        if (device.deviceInfo.deviceId === deviceId) {
          const newElement = { deviceInfo: device.deviceInfo, checked: true }
          upDateDeviceList.push(newElement)
        } else {
          const newElement = { deviceInfo: device.deviceInfo, checked: false }
          upDateDeviceList.push(newElement)
      ...
      setSeletedDeviceID(deviceId)
    }

    if (deviceId) {
      setIsQrayDevice(true)
      console.log("Found a valid QRAY device")
    } else {
      setIsQrayDevice(false)
  ...
  useInterval(() => {
    ...
    if (selectedDeviceID) {
      ...
      getQrayStream(selectedDeviceID)
    } else {
      getQrayDevices()
    }
  }, 2000)
